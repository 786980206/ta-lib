...
The Rust FFI crate (`rust/ffi/`) exports `extern "C"` wrappers matching the C function signatures. A separate CMake target (`ta_regtest_rust`) links ta_regtest against the Rust staticlib instead of the C implementations. ta_regtest's test logic is completely unchanged — it just calls into Rust code via FFI.

Global state (unstable periods, compatibility mode) is managed by the C library. Each Rust FFI wrapper reads the current state from C globals via `TA_GetUnstablePeriod()`/`TA_GetCompatibility()` before calling the Rust implementation.

### AI Tight-Loop Workflow

When porting a new function to Rust:
1. Add function name to `RUST_SUPPORTED_FUNCS` in `gen_code.c`
2. Convert the function's C source to cross-language macros
3. Run gen_code to generate Rust code + FFI wrapper
4. Fix compilation errors: `cd rust/ffi && cargo build --release`
5. Test: `cd bin && ./ta_regtest_rust --function=FUNCTION_NAME`
6. Fix and repeat from step 4 until ta_regtest passes
...
FFI wrappers are auto-generated by gen_code (see `writeRustFfiGenerated()` in `gen_rust.c`). When adding a new function to `RUST_SUPPORTED_FUNCS`, the FFI wrapper is generated automatically — no hand-writing needed.

Single-precision FFI wrappers (`TA_S_*`) widen f32 inputs to f64 at the FFI boundary and call the generic function with f64. This matches the C behavior where single-precision wrappers convert to double internally.

Output: `rust/ffi/src/generated.rs`

### Key Files

| File | Purpose |
|------|---------|
| `rust/ffi/Cargo.toml` | FFI staticlib crate config |
| `rust/ffi/src/lib.rs` | Re-exports generated wrappers |
| `rust/ffi/src/generated.rs` | Auto-generated extern "C" wrappers |
| `src/tools/gen_code/gen_rust.c` | FFI generation functions |
| `CMakeLists.txt` | `ENABLE_RUST_REGTEST` flag and `ta_regtest_rust` target |
