...
/// Generate `ta_func_private.h` — forward declarations for the private function
/// variants: double `TA_*_Private` and single-precision `TA_S_*_Private` (custom
/// signatures, e.g. EMA's k factor; the SP variant takes `const float` inputs with
/// double intermediates/outputs). Emitted for every `has_explicit_private` function
/// so a declaration can never lag the definition emitted by `backends::c::gen_private`.
