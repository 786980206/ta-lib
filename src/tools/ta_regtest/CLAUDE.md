...
| File | Purpose |
|------|---------|
| `ta_regtest.c` | Main entry point. CLI flags: `--function=CSV`, `--codegen`, `-p` (profile) |
...
3 hand-coded callbacks (SMA, MULT, RSI) with per-function JSON request building. Need to generalize to all 158 indicators via ta_abstract.
...
| `TA_OptInput_RealRange` | BBANDS `optInNbDevUp`, SAR `optInAcceleration` | Needs `json_find_double` (currently broken — uses int) |
