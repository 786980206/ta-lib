...
pub fn generate_c_header_stub(funcs: &[FuncDef]) -> String {
    ...
    for func in funcs {
        ...
        // TA_INT_* macros are defined per .c file, not in the header
        ...
        // TA_S_INT_* macros are defined per .c file, not in the header
    }
    s.push('\n');

    // TA_INT_* macros — must be defined BEFORE ta_utility.h is included
    // (by the individual .c files) so that ta_utility.h's TA_INT_* declarations
    // expand to the correct _Unguarded function names.
    s.push_str("/* TA_INT_* macros — map legacy internal names to _Unguarded functions */\n");
    for func in funcs {
        let upper = func.name.to_uppercase();
...
pub fn generate_c_server(funcs: &[FuncDef]) -> String {
    ...
    // Include ta_func_unguarded.h first — defines TA_INT_* macros that
    // ta_utility.h declarations need to expand correctly.
