...
/* Candle settings globals — defined here, declared extern in ta_lib_types.h.
 * Must NOT be static so the compiler doesn't constant-propagate through the
 * pointer, which prevents loop unswitching in CDL pattern functions. */
